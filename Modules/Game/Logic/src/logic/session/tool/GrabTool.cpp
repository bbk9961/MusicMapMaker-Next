...
{
    if ( cmd.entity != entt::null && session.m_noteRegistry.valid(cmd.entity) ) {
        ...
        if ( !session.m_noteRegistry.all_of<InteractionComponent>(cmd.entity) ) {
            session.m_noteRegistry.emplace<InteractionComponent>(cmd.entity);
        }
        session.m_noteRegistry.get<InteractionComponent>(cmd.entity).isDragging = true;

        if ( auto* note = session.m_noteRegistry.try_get<NoteComponent>(cmd.entity) ) {
            session.m_dragInitialNote = *note;
        }
    }
}

void GrabTool::handleUpdateDrag(BeatmapSession& session, const CmdUpdateDrag& cmd)
{
                ...
                if ( auto* note = session.m_noteRegistry.try_get<NoteComponent>(
                    ...
                    if ( note->m_type == ::MMM::NoteType::HOLD &&
                        ...
                        note->m_duration = newDuration;
                                ...
                                session.m_draggedPart == HoverPart::FlickArrow ) {
                        ...
                        targetTrack =
                            std::clamp(targetTrack, 0, session.m_trackCount - 1);
                        ...
                        if ( auto* trans =
                                 session.m_noteRegistry.try_get<TransformComponent>(
                                     session.m_draggedEntity) ) {
...
void GrabTool::handleEndDrag(BeatmapSession& session, const CmdEndDrag& cmd)
{
    if ( session.m_draggedEntity != entt::null &&
         session.m_noteRegistry.valid(session.m_draggedEntity) ) {
        if ( session.m_noteRegistry.all_of<InteractionComponent>(session.m_draggedEntity) ) {
            session.m_noteRegistry.get<InteractionComponent>(session.m_draggedEntity)
                .isDragging = false;
        }
        ...
        if ( session.m_dragInitialNote.has_value() ) {
            auto* currentNote =
                session.m_noteRegistry.try_get<NoteComponent>(session.m_draggedEntity);
            if ( currentNote ) {
                auto action =
                    std::make_unique<NoteAction>(NoteAction::Type::Update,
                                                 ...
                                                 *currentNote);
                session.m_actionStack.pushAndExecute(std::move(action), session);
            }
        }
